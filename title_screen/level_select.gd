...
func _process(delta):
	if not is_dirty:
		return
	var level_count: int = SceneManager.scenes.size() - level_offset
	var button_count := $GridContainer.get_child_count()
	for i in button_count:
		var button := $GridContainer.get_child(i) as Button
		var level_index := i + current_page * button_count + level_offset
...
func _on_button_next_page_pressed():
	var pages: float = (SceneManager.scenes.size() - level_offset) / float($GridContainer.get_child_count())
